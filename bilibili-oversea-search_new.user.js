...
(function () {
  ...
  function searchTH(page) {
    ...
    const tag = GM_getValue("search_page_style_tag"),
      list = document.querySelector(".search-content"),
      searchPage = list.querySelector(".search-page"),
      nodataNode = searchPage.querySelector(".search-nodata-container"),
      loadingNode = searchPage.querySelector(".vui_loading-body"),
      neterrorNode = searchPage.querySelector(".search-neterror-container"),
      mediaListNode = searchPage.querySelector(".media-list"),
    ...
    GM_xmlhttpRequest({
      method: "GET",
      url: `https://${GM_getValue("th_search_proxy_server")}/intl/gateway/v2/app/search/type?${Object.entries(
        query,
      )
      ...
      onload: (e) => {
        ...
        if (!result.items) {
          nodataNode.classList.remove("hide");
          loadingNode.classList.add("hide");
          mediaListNode.classList.add("hide");
          neterrorNode.classList.add("hide");
          return;
        }
...
<picture class="v-img" ${tag.card} ${tag.vimg || ""}>\
<source srcset="${item.cover.replace(/https?:/, "")}" type="image/webp" ${tag.vimg || ""}>\
<img src="${item.cover.replace(/https?:/, "")}" loading="lazy" onload="" ${tag.vimg || ""}>\
</picture>\
...
<div title="${item.desc}" class="media-card-content-head-text media-card-content-head-desc text_ellipsis_3l" ${tag.card}><div>简介:</div><span>${item.desc}</span></div>\
        ...
        neterrorNode.classList.add("hide");
        nodataNode.classList.add("hide");
        mediaListNode.classList.remove("hide");
  ...
  function searchTHM(page) {
    ...
    GM_xmlhttpRequest({
      method: "GET",
      url: `https://${GM_getValue(area + "_search_proxy_server") || "api.bilibili.com"}/x/web-interface/search/type?${Object.entries(
        query,
      )
        .map(([key, value]) => `${key}=${value}`)
        .join("&")}`,
      ...
      onload: (e) => {
        ...
        for (let item of result.result) {
          const ep = item.eps
            ? item.eps.map(
                (ep) =>
                  `<a href="https://www.bilibili.com/bangumi/play/ep${ep.id}" target="_blank" class="p_relative" ${tag.footer}><button class="vui_button media-footer-btn" ${tag.footer}>${Number(ep.title) ? ep.title : ep.title}</button></a>`,
              )
            : [];
          ...
          const score =
            item.media_score && item.media_score.user_count > 0
              ? `<div class="score media-card-content-footer-score" ${tag.score} ${tag.card}><span class="score-text" ${tag.score}>${item.media_score.user_count < 10000 ? item.media_score.user_count : `${(item.media_score.user_count / 10000).toFixed(1)}万`}人评分</span><span class="score-value" ${tag.score}>${item.media_score.score}<span >分</span></span></div>`
              : "";
          const resultNode = document.createElement("div");
...
<div class="media-footer" ${tag.card} ${tag.footer}>${ep.join("")}</div>\
  ...
  function pager(page, total) {
    ...
    const searchPage = searchContent.querySelector(".search-page");
    const mediaListNode = searchPage.querySelector(".media-list");
    const parentNode = mediaListNode.parentNode;
    parentNode.querySelector(".vui_pagenation")?.parentNode.remove();
    if (total <= 20) return;
    ...
    if (page - 1 <= 4) {
      ...
      pagesNodes +=
        '<button class="vui_button vui_button--no-transition vui_pagenation--btn vui_pagenation--btn-num">1</button><span class="vui_pagenation--extend">...</span>';
    ...
    document
      .querySelectorAll(
        ".search-content .vui_pagenation .vui_button:not(.vui_button--active):not(.vui_button--disabled)",
      )
      .forEach((node) =>
  ...
  function switchToNormal() {
    ...
    const searchPage = list.querySelector(".search-page");
    const nodataNode = searchPage.querySelector(".search-nodata-container");
    nodataNode.classList.add("hide");
    const mediaListNode = searchPage.querySelector(".media-list");
    if (mediaListNode) {
      mediaListNode.classList.add("hide");
    ...
    const neterrorNode = searchPage.querySelector(".search-neterror-container");
    if (neterrorNode) neterrorNode.classList.add("hide");
    const loadingNode = searchPage.querySelector(".vui_loading-body");
  ...
  function createTemplates() {
    ...
    loadingTpl.innerHTML =
      '<div class="vui_loading-loading-tv" style="--v-loading-size: 1.2;"><img width="24" height="24" src="//s1.hdslb.com/bfs/static/jinkela/long/gif/loading-blue.gif"></div><div class="vui_loading-description">加载中...</div>';
    MyTemplates.loading = loadingTpl;
    const tag = GM_getValue("search_page_style_tag");
    const nodataTpl = document.createElement("div");
    nodataTpl.className = "search-nodata-container p_relative";
    nodataTpl.setAttribute(tag.nodata, "");
    nodataTpl.innerHTML = `<div ${tag.nodata} class="no-data p_center text_center"><img ${tag.nodata} class="no-data-img" src="//i0.hdslb.com/bfs/static/laputa-search/assets/nodata.67f7a1c9.png" alt=""><p ${tag.nodata} class="mt_lg b_text text3">今天真是寂寞如雪啊~</p></div>`;
    MyTemplates.nodata = nodataTpl;
    const neterrorTpl = document.createElement("div");
    neterrorTpl.className = "search-neterror-container p_relative";
    neterrorTpl.setAttribute(tag.neterror, "");
  ...
  function initCustomSearch() {
    ...
    const searchPage = list.querySelector(".search-page");
    const nodataNode = searchPage.querySelector(".search-nodata-container");
    if (nodataNode) nodataNode.classList.add("hide");
    else {
      const nodataNewNode = MyTemplates.nodata.cloneNode(true);
      nodataNewNode.classList.add("hide");
      searchPage.appendChild(nodataNewNode);
    }
    const mediaListNode = searchPage.querySelector(".media-list");
    if (mediaListNode) {
      mediaListNode.classList.add("hide");
      ...
      const mediaListNewNode = MyTemplates.mediaList.cloneNode(true);
      searchPage.appendChild(mediaListNewNode);
    }
    const neterrorNode = searchPage.querySelector(".search-neterror-container");
    if (neterrorNode) neterrorNode.classList.add("hide");
    else {
      const neterrorNewNode = MyTemplates.neterror.cloneNode(true);
      neterrorNewNode.classList.add("hide");
      searchPage.appendChild(neterrorNewNode);
    }
    const loadingNode = searchPage.querySelector(".vui_loading-body");
    if (loadingNode) loadingNode.classList.remove("hide");
    else {
      const loadingNewNode = MyTemplates.loading.cloneNode(true);
      searchPage.appendChild(loadingNewNode);
    }
  }
  ...
  function injectButton(injected) {
    const injectNode =
        /*document.querySelector(injected ? '#bangumi-list.inject-node .total-wrap' : '#bangumi-list .total-wrap')*/ document.querySelector(
          ".search-header",
        ),
    ...
    proxy_area.innerHTML =
      '<select id="proxy-area"><option value="cn">大陆</option><option value="hk">港澳</option><option value="tw">台湾</option><option value="th">海外</option></select>';
  ...
  function injectObserver() {
    ...
    let observeNode;
    document
      .querySelectorAll(".search-tabs > .vui_tabs > nav > ul > li")
      .forEach(
        (node) => node.innerHTML.includes("番剧") && (observeNode = node),
      );
  ...
  const searchAppObserver = new MutationObserver((mutationsList) => {
    for (let mutation of mutationsList) {
      ...
      if (
        ...
        mutation.addedNodes[0].querySelector(".search-content")
      ) {
        injectObserver();
        document
  ...
  searchAppObserver.observe(document.querySelector(".search-layout"), {
    childList: true,
  });
  ...
  if (
    styleLink &&
    styleLink[1] &&
    styleLink[1] !== GM_getValue("search_page_style_hash")
  ) {
    GM_xmlhttpRequest({
      method: "GET",
      ...
      onload: (e) => {
        ...
        if (styles) {
          const card = (styles.match(/\.media-card\[(data-v-[^\]]+)\]/) ??
            [])[1];
          const neterror = (styles.match(
            /\.search-neterror-container\[(data-v-[^\]]+)\]/,
          ) ?? [])[1];
          const nodata = (styles.match(
            /\.search-nodata-container\[(data-v-[^\]]+)\]/,
          ) ?? [])[1];
          const score = (styles.match(/\.score\[(data-v-[^\]]+)\]/) ?? [])[1];
          const tag = (styles.match(/\.tag\[(data-v-[^\]]+)\]/) ?? [])[1];
          const footer = (styles.match(/\.media-footer\[(data-v-[^\]]+)\]/) ??
            [])[1];
          const vimg = (styles.match(/\.v-img\[(data-v-[^\]]+)\]/) ?? [])[1];
          const search_page_style_tag = {
            card,
            neterror,
            nodata,
            score,
